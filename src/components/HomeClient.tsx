...
export default function HomeClient({ 
  ...
  const sectionTitle = homeData?.featuredSectionTitle || "The Roseline Ring";
  const sectionSubtitle = homeData?.featuredSectionDescription || "Elegance in Every Detail";
  const parallaxText = featuredSectionProduct?.name?.toUpperCase() || "ROSELINE";
  const featuredImage = featuredSectionProduct?.imageUrl || "/luxury-ring-cutout.png";
  ...
  return (
    <main className="min-h-screen">
      <Hero 
        ...
        italicTitle={homeData?.heroTitleItalic} 
      />
      <BentoGrid blocks={homeData?.bentoBlocks} />
      <OurWorks />
      ...
      <section ref={containerRef} className="relative h-[300vh] bg-background">
        <div className="sticky top-0 h-screen w-full flex flex-col items-center justify-center overflow-hidden">
          ...
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none select-none">
            <motion.h2 
              style={{ x: textX, opacity: 0.05 }}
              className="text-[25vw] font-bold text-foreground tracking-tighter whitespace-nowrap leading-none"
            >
          ...
          >
            <h2 className="text-4xl md:text-6xl font-serif text-foreground tracking-[0.2em] uppercase text-center">
              {sectionTitle}
            </h2>
          </motion.div>
          ...
          <div className="relative w-full max-w-2xl aspect-square flex items-center justify-center">
            ...
            {/* Decorative Glow */}
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-80 md:w-[35rem] h-80 md:h-[35rem] bg-gold/10 blur-[150px] rounded-full z-10" />
          </div>
